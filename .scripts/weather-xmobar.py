...
def wrap_xmobar_color(color: str, data: str) -> str:
    return "<fc=" + color + "><fn=4>" + data + "</fn></fc>"


# function to assemble weather reporting string for xmobar
def make_xmobar_weather_string(
    weather_map: dict, color_map: dict, wind_color_map: dict
) -> str:
    days_time = day_or_night(weather_map)
    report_string = ""
    try:
        if days_time == "day":
            report_string += (
                wrap_xmobar_color(color_map["weather_day"], return_icon(weather_map))
                + " "
            )
        elif days_time == "night":
            report_string += (
                wrap_xmobar_color(color_map["weather_night"], return_icon(weather_map))
                + " "
            )
        else:
            report_string += (
                wrap_xmobar_color(color_map["unknown"], return_icon(weather_map)) + " "
            )
        report_string += (
            wrap_xmobar_color(color_map["realTemp"], weather_map["temp_C"]) + "("
        )
        report_string += (
            wrap_xmobar_color(color_map["feelsLikeTemp"], weather_map["FeelsLikeC"])
            + ")°C "
        )
        report_string += (
            wrap_xmobar_color(
                color_map["windDirection"],
                wind_directions.get(weather_map["winddir16Point"], ""),
            )
            + ":"
        )
        if int(weather_map["windspeedMs"]) < int(weather_map["windgustMs"]):
            ...
            report_string += (
                wrap_xmobar_color(
                    give_wind_color(wind_color_map, int(weather_map["windgustKmph"])),
                    weather_map["windgustMs"],
                )
                + "m/s "
            )
        else:
...
def main():
    ...
    try:
        ...
        current_weather_map["sunrise"] = convert24(
            response["weather"][0]["astronomy"][0]["sunrise"]
        )
        current_weather_map["sunset"] = convert24(
        ...
        weather_report = make_xmobar_weather_string(
            current_weather_map, weather_colors, wind_colors
        )
    except requests.RequestException:
        weather_report = wrap_xmobar_color(
            weather_colors["error"], "N/A, Connection issue"
        )
